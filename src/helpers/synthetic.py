...
async def generate_content_async(
    ...
    cmd = [
        "synthetic-data-kit", "-c", str(config_path),
        "create", chunk_file,
        "--type", gen_type,
    ]
...
async def curate_content_async(
    ...
    if result.returncode != 0:
        if log_fn:
            await log_fn(f"    ⚠️ Curation error, using original")
        return json_file
    ...
    try:
        for idx, source in enumerate(sources):
            ...
            for i, chunk_file in enumerate(chunks_to_process):
                ...
                gen_file = await generate_content_async(
                    chunk_file, config_path, gen_type, num_pairs, log
                )
                if not gen_file:
                ...
                if curate:
                    gen_file = await curate_content_async(
                        gen_file, config_path, curate_threshold, log
                    )
        ...
        if all_ft_files:
            ...
            conversations = pd.concat([
                pd.read_json(f) for f in all_ft_files
