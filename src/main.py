...
"""
    ...
    def _on_infer_adapter_picked(e):
        try:
            sel = getattr(e, "path", None) or ""
            if sel:
    ...
    def on_infer_use_latest(e=None):
        try:
            info = train_state.get("local_infer") or {}
        except Exception:
            info = {}
        try:
            adapter_path = (info.get("adapter_path") or "").strip()
            base_model_name = (info.get("base_model") or infer_base_model_tf.value or "unsloth/Meta-Llama-3.1-8B-Instruct-bnb-4bit").strip()
            if adapter_path and os.path.isdir(adapter_path):
    ...
    def _close_infer_chat_dialog(e=None):
        try:
            try:
    ...
    def _set_infer_controls_enabled(enabled: bool):
    ...
    def on_infer_clear(e=None):
        try:
            _reset_infer_history()
        except Exception:
            pass

    def on_infer_preset_change(e=None):
        try:
    ...
    async def on_infer_validate_adapter(e=None):
        ...
        if (not adapter_path) or (not os.path.isdir(adapter_path)):
            ...
            _set_infer_controls_enabled(False)
            try:
                page.snack_bar = ft.SnackBar(ft.Text("Invalid adapter directory. Please choose a valid adapter folder."))
                page.open(page.snack_bar)
            except Exception:
                pass
            await safe_update(page)
        ...
        if not _looks_like_adapter_dir(adapter_path):
            ...
            _set_infer_controls_enabled(False)
            try:
                page.snack_bar = ft.SnackBar(ft.Text("Adapter directory doesn't look like a valid LoRA adapter."))
                page.open(page.snack_bar)
            except Exception:
                pass
            await safe_update(page)
    ...
    async def on_infer_generate(e=None):
        ...
        loaded = bool(info.get("model_loaded")) and (
            ...
            (info.get("base_model") or "").strip() == base_model_name
        )
    ...
    def on_infer_open_full_chat(e=None):
    ...
    async def on_infer_chat_send(e=None):
