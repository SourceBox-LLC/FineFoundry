...
def list_training_configs(
    train_target: Optional[str] = None,
...
def get_training_config(name: str, db_path: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    cursor.execute(
        "SELECT config_json FROM training_configs WHERE name = ?",
        (name,)
    )
    row = cursor.fetchone()
...
def rename_training_config(
    ...
    cursor.execute(
        "UPDATE training_configs SET name = ?, updated_at = datetime('now') WHERE name = ?",
        (new_name, old_name)
    )
    conn.commit()
